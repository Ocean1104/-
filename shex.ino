...
int PIR = 9;              // Define DIGITAL 9 as PIR (Passive Infrared Sensor)
int led = 13;             // Define DIGITAL 13 as led (LED module)
int buzzer = 10;          // 定义数字引脚10为蜂鸣器模块
int time = 0;
...
void setup(){
    pinMode(led, OUTPUT);   // Set led as digital output
  pinMode(PIR, INPUT);    // Set PIR as digital input
  pinMode(buzzer, OUTPUT);  // 设置蜂鸣器模块为数字输出
...
void loop(){
  captureImg(320, 240);
   if (digitalRead(PIR))   // If someone passes through
  {
    digitalWrite(led, HIGH); // Turn on the LED module
  ...
  {
    digitalWrite(led, LOW); // Turn off the LED module
